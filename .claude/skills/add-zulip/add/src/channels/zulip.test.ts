...
describe('ZulipChannel', () => {
  it('connects and registers the event queue', async () => {
    ...
    const fetchMock = vi.fn((url: string) => {
      ...
      if (url.includes('/register')) {
        return mockJsonResponse({ result: 'success', queue_id: 'q1', last_event_id: 1 });
      }
      ...
      if (url.includes('/events')) {
        ...
        return mockJsonResponse({ result: 'success', events: [], last_event_id: 1 });
      }
  ...
  it('delivers stream messages with topic metadata', async () => {
    ...
    const fetchMock = vi.fn((url: string) => {
      ...
      if (url.includes('/events')) {
        ...
        return mockJsonResponse({ result: 'success', events: [], last_event_id: 2 });
      }
      return mockJsonResponse({ result: 'success' });
    });
    globalThis.fetch = fetchMock as typeof fetch;
  ...
  it('sends messages using topic from JID', async () => {
    const fetchMock = vi.fn(() =>
      mockJsonResponse({ result: 'success', id: 123 }, 200),
    );
    globalThis.fetch = fetchMock as typeof fetch;
