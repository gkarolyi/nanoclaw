...
describe('ZulipChannel', () => {
  ...
  describe('connection lifecycle', () => {
    ...
    it('throws on auth failure', async () => {
      (global.fetch as any).mockResolvedValueOnce({
        json: () => Promise.resolve({ result: 'error', msg: 'Invalid API key' }),
      });
    ...
    it('disconnects cleanly', async () => {
      (global.fetch as any)
        .mockResolvedValueOnce({
          json: () =>
            Promise.resolve({ user_id: 123, full_name: 'Andy Bot' }),
        })
        .mockResolvedValueOnce({
          json: () =>
            Promise.resolve({
  ...
  describe('message handling', () => {
    ...
    it('tracks last topic per stream', () => {
      ...
      expect((channel as any).lastTopicByStream.get('42')).toBe(
        'second-topic',
      );
    });
  });
