...
function buildThreadFolder(
  ...
  const slug = slugifyThreadName(threadName || threadId);
  const separator = '__';
  let candidate = `${parent.folder}${separator}${slug}`;
  if (candidate.length > THREAD_FOLDER_MAX) {
    const hash = threadHash(parent, threadId);
    const maxPrefix = THREAD_FOLDER_MAX - separator.length - hash.length;
    const prefix = parent.folder.slice(0, Math.max(1, maxPrefix));
    candidate = `${prefix}${separator}${hash}`;
  }
  if (!isValidGroupFolder(candidate)) {
    const fallback = `thread_${threadHash(parent, threadId)}`;
    ...
    onMessage: (chatJid: string, msg: NewMessage) => {
      ...
      if (msg.thread_id) {
        ...
        if (!registeredGroups[targetJid]) {
          const threadGroup = buildThreadGroup(
            parentGroup,
            msg.thread_id,
            msg.thread_name,
          );
      ...
      const allowlistGroup = parentGroup ?? registeredGroups[targetJid];
      // Sender allowlist drop mode: discard messages from denied senders before storing
      if (!msg.is_from_me && !msg.is_bot_message && allowlistGroup) {
        const cfg = loadSenderAllowlist();
        const allowlistJid = parentGroup ? chatJid : targetJid;
        if (
          ...
          if (cfg.logDenied) {
            logger.debug(
              { chatJid: allowlistJid, sender: msg.sender },
      ...
      storeMessage({ ...msg, chat_jid: targetJid });
    },
    onChatMetadata: (
      chatJid: string,
      timestamp: string,
