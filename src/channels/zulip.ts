...
function parseZulipJid(
  jid: string,
): { streamId: number; topic?: string } | null {
...
export class ZulipChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    logger.info(
      { site: this.creds.site, email: this.creds.email },
      'Zulip connected',
    );
  }
  ...
  private async registerEventQueue(): Promise<void> {
    const response = await this.request<ZulipRegisterResponse>(
      '/register',
      'POST',
      {
        event_types: ['message'],
    ...
    if (!response.queue_id) {
      throw new ZulipApiError('Zulip register response missing queue_id');
    }
    this.queueId = response.queue_id;

    const parsedLastEventId = Number(response.last_event_id);
    if (!Number.isFinite(parsedLastEventId)) {
      logger.warn(
        { lastEventId: response.last_event_id },
        'Zulip register response missing last_event_id; defaulting to 0',
      );
      this.lastEventId = 0;
      return;
    }
    this.lastEventId = parsedLastEventId;
  }
  ...
  private async pollEvents(): Promise<void> {
    while (this.polling) {
      ...
      if (!this.queueId) {
        logger.warn('Zulip event queue missing after registration, retrying');
        await sleep(2000);
        continue;
      }
      const lastEventId = Number.isFinite(this.lastEventId)
        ? this.lastEventId
        : 0;
      try {
        const response = await this.request<ZulipEventResponse>(
          '/events',
          'GET',
          {
        ...
        const nextEventId = Number(response.last_event_id);
        if (Number.isFinite(nextEventId)) {
          this.lastEventId = nextEventId;
        } else {
          const events = response.events ?? [];
          const fallbackEventId = events.length
            ? Number(events[events.length - 1]?.id)
            : NaN;
          if (Number.isFinite(fallbackEventId)) {
            this.lastEventId = fallbackEventId;
            logger.debug(
              { lastEventId: response.last_event_id, fallbackEventId },
              'Zulip events response missing last_event_id; using last event id',
            );
          } else {
            logger.warn(
              { lastEventId: response.last_event_id },
              'Zulip events response missing last_event_id; keeping previous value',
            );
